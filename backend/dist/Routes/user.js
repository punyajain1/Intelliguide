        ...
        if (response.data.status === 'OK') {
            ...
            try {
                ...
                try {
                    ...
                    const chatCompletion = yield index_1.client.chat.completions.create({
                        messages: [
                            {
                                "role": "system",
                            ...
                            {
                                role: 'user',
                                content: `Write a short summary about me from the following Codeforces account information of myself: ${JSON.stringify(data)}.(not more then 1 peragraph and write in second person)`,
                            },
                        ],
                ...
                if (data[0].rating < 2100) {
                    try {
                        yield new Promise(resolve => setTimeout(resolve, 500));
                        const chat = yield index_1.client.chat.completions.create({
                            messages: [
                                {
                                    "role": "system",
                                    "content": promts_1.PROFILE_ANALYZER_PROMPT,
